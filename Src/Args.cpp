...
void OptParse::RegisterOption(const std::string flag, double & value, const unsigned char shorthand, const bool required) {
    RegisterOptionGeneric(flag,&value,shorthand,required,DoublePrecisionFloatingPoint);
}
                ...
                switch (types_[i]) {
                    ...
                    case FloatingPoint:
                        *reinterpret_cast<float *>(valuePtrs_[i]) = atof(optarg);
                        break;
                    case DoublePrecisionFloatingPoint:
                        *reinterpret_cast<double *>(valuePtrs_[i]) = atof(optarg);
                        break;
                    case Boolean:
